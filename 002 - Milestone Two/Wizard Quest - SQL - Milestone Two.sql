						...
						if verifyUser is null then -- invalid password                       
							...
							where UserName = pUserName;
								if (select UserID -- failed logins less than five
                                    ...
                                    set UserLocked = true
                                    where UserName = pUserName;
									select 'Account Locked - Contact Administator' as message;
								end if;
                        else
							if (select UserID -- valid username and password but account is locked
								...
								where UserName = pUserName;
								select 'Login Success - User Online' as message;
							end if;
                        end if;
				end;
        else -- username does not exists
			select 'Invaild Username' as message;
		end if;
...
delimiter //
create procedure userLogout(in pUserID int)
begin
	update tblUser
    set UserOnline = false
    where UserID = pUserID;
    select 'Logout Sucess - User Offline' as message;
end //
delimiter ;
...
begin
    ...
    into verifyUser;
		...
		if verifyUser is null then
			...
			delete from tblUser
            where UserName = pUserName;
			select 'Delete Sucess - User Deleted' as message;
		end if;
end //
delimiter ;
